...
Join the participant data to the AUC data (by `id` and `stim`, since each participant has observations for each stimulus).
...
### Summarize precision by age group
...
### Facet by `age_group` with free scales
...
### Create a summary table of age in years by age group
